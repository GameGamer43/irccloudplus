...
prefs.chan_highlight = function () {
	 try {
	 	var list = JSON.parse(localStorage.chan_highlight);
	} catch (e) {
		list = [localStorage.chan_highlight];
	}
	console.log('list', localStorage.chan_highlight);
	list.each(function(i, chan){
		if (chan.charAt(0) != '#') {
			list[i] = '#' + chan;
		}
	});
	...
	switch (msg.type) {
		case 'buffer_msg':
			if (msg.highlight && prefs.playalert()) {
				prefs.beep.play();
			} else if ($.inArray(msg.chan, prefs.chan_highlight()) > -1) {
